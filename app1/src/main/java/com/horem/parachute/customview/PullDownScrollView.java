...
public class PullDownScrollView extends LinearLayout {
	...
	public boolean onInterceptTouchEvent(MotionEvent ev) {
		...
		if (ev.getAction() == MotionEvent.ACTION_MOVE) {
			...
			// Logger.d(TAG, "isElastic:" + isElastic + " canScroll:"+
			// canScroll() + " ev.getY() - preY:"+(ev.getY() - preY));
			if (!isElastic && canScroll() && (int) ev.getY() - preY >= headContentHeight / (3 * RATIO) && refreshListener != null && mElastic != null) {
